...
\level{3}{Fase SD: Software Design}
	\textbf{Periodo}: dal \insdate{05}{03}{2015} al \insdate{03}{04}{2015} \\Questa \insglo{fase} comincia con la fine della \insphase{Fase DI} e termina con l'incontro con il proponente per mostrare l'architettura scelta.\\
	Le attività di questa \insglo{fase} sono:
	\begin{itemize}
		...
		\item \textbf{Specifica Tecnica}: Questa attività caratterizza la \insphase{Progettazione Architetturale}. Il \insrole{Progettista} stende la \insdoc{Specifica Tecnica} che contiene le scelte progettuali, ad alto livello, che il progetto dovrà avere. Saranno quindi descritti quali \insglo{design pattern} \projectname{} implementerà, l'architettura generale del \insglo{software}, i principali flussi di controllo e il tracciamento dei requisiti.
		...
		\item \textbf{Piano di Progetto}: l'incremento che sarà fatto al documento \insdoc{Piano di Progetto} in questa \insglo{fase} consiste nell'apportare correzioni nella divisione delle attività e stillare il consuntivo di questo periodo. Dopo un'accurata verifica che fisserà una nuova baseline e la validazione il documento diventerà \insdoc{Piano di Progetto v3.00}.
	\end{itemize}
