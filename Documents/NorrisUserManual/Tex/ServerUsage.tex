	...
	\level{3}{Initialization}
		\begin{itemize}
			...
			\begin{enumerate}
				...
				\end{lstlisting}
				\item Create a new instance of \texttt{\insglo{http}.\insglo{Server}}:
				\begin{lstlisting}
					var \insglo{server} = \insglo{http}.createServer();
				\end{lstlisting}
				\texttt{\insglo{server}} will emit events you can listen. For example the event \texttt{'request'} is emitted each time there is a request.
			...
			\begin{enumerate}
				...
				\end{lstlisting}
				\item Create a new instance of \insglo{Norris}, and make it accessible by a \insglo{client} application at the given endpoint (e.g. \texttt{'/\insglo{norris}'}):
				\begin{lstlisting}
