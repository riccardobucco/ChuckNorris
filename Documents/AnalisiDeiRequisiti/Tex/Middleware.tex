...
\level{1}{Middleware in Express.js} \label{app:express}
	\level{2}{Cos'è un middleware?}
		...
		Riportiamo in seguito la descrizione che fornisce il sito \url{expressjs.com} di \insglo{middleware} (riportata in lingua originale):
		\begin{quote}
	...
	\level{2}{Ottenere un middleware usando le API interne di Norris}
		...
		\begin{quote}
			l'uso del \insglo{framework} (\insglo{Norris} ndr) dovrà essere compatibile con l’utilizzo standard dei \insglo{middleware} di Express, versione 4.x (\url{\insglo{http}://expressjs.com/4x/\insglo{api}.html#\insglo{middleware}})
		\end{quote}
