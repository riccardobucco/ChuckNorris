\level{2}{Processo di documentazione}
	...
	La stesura dei documenti deve essere fatta nel modo più accurato e completo possibile, in modo da ridurre i tempi nella \insglo{fase} di verifica.
		\level{3}{Norme}
			\level{4}{Codifica dei file}
				Per quanto riguarda la codifica dei file si rimanda alla sezione \nameref{sec:CodificaFile} del processo di configurazione.
			...
			\level{4}{Norme tipografiche}
				\level{5}{Formattazione generale}
					\level{6}{Testatine}
						...
						\begin{itemize}
							\item logo del \insglo{team}, posizionato a sinistra;
				...
				\level{5}{Formati ricorrenti} \label{sec:formatiricorrenti}
					\level{6}{Date}\label{sec:formatoDate}
					...
					\level{6}{URI}
						Per fare riferimento a un \insglo{URI} è necessario utilizzare il comando \LaTeX{} apposito: \textbackslash insuri\{nome\_uri\}. Esso garantisce la corretta scrittura dell'\insglo{URI} secondo le norme seguenti:
						\begin{itemize}
							\item si adotta lo stile \insglo{monospace};
					...
					\level{7}{Nomi dei file}
						...
						\end{itemize}
					\level{7}{Etichette issue} \label{sec:sintassiissue}
						Per fare riferimento ad una label di \insglo{issue} è necessario utilizzare il comando \LaTeX{} apposito: \textbackslash inslabel\{NomeLabel\}. Esso garantisce la corretta scrittura dell'etichetta rispetto le norme seguenti:
						\begin{itemize}
					...
					\level{6}{Fasi del progetto}
						Per fare riferimento a una \insglo{fase} di progetto è necessario utilizzare il comando \LaTeX{} apposito: \textbackslash insphase\{nome\_fase\}. Esso garantisce la corretta scrittura della \insglo{fase} di progetto secondo le norme seguenti:
						\begin{itemize}
							\item la prima lettera di ogni parola è maiuscola, eccetto le preposizioni;
							\item si adotta lo stile grassetto.
						\end{itemize}
			...
			\level{4}{Componenti grafiche}
				\level{5}{Immagini}
					...
					\begin{itemize}
						\item gli unici formati ammessi sono il \insglo{PDF} e il \insglo{PNG};
				...
				\level{5}{Diagrammi UML} \label{sec:UML}
					...
					\item il nome dei casi d'uso deve essere formato dalla sigla appropriata del caso d'uso, indicata nella sezione \nameref{sec:classificazioneUC}, dal numero gerarchico e, dopo uno spazio, dal nome descrittivo del caso d'uso;
					...
					\end{itemize}
					\level{6}{Diagrammi delle attività}
					...
					\end{itemize}
					\level{6}{Diagrammi dei package}
					Ogniqualvolta sia necessario utilizzare i diagrammi dei \insglo{package} devono essere rispettate le seguenti norme:
					\begin{itemize}
					\item i termini devono essere scritti in lingua inglese.
					\item il nome dei \insglo{package} può essere costituito da più parole, le quali possono essere formate solo da lettere minuscole o maiuscole e devono usare il formato CamelCase.
					\end{itemize}
					\level{6}{Diagrammi delle classi}
					Ogniqualvolta sia necessario utilizzare i diagrammi delle classi devono essere rispettate le seguenti norme:
					\begin{itemize}
					\item il nome completo di una classe è formato dal nome di un \insglo{package}, seguito da '::' e dal nome proprio della classe;
			...
			\level{4}{Tipologie dei documenti}
				...
				\level{5}{Documenti formali}
					Un documento risulta formale quando viene approvato dal Responsabile Di Progetto. Solo i documenti formali possono essere forniti a soggetti esterni al \insglo{team}.\\
		...
		\level{3}{Procedure}
			\level{4}{Procedura di formalizzazione dei documenti}
				Di seguito viene riportata la \insglo{procedura} alla quale deve essere sottoposto un documento prima di essere definitivamente formalizzato:
				\begin{enumerate}
					...
					\item il \insrole{Responsabile di Progetto} può decidere di approvare il documento (e in questo caso la \insglo{procedura} di formalizzazione termina, documento approvato) oppure di respingerlo (in questo caso si torna al primo punto).
				\end{enumerate}
				La \insglo{procedura} è formalizzata nel seguente diagramma delle attività:
				\begin{figure}[H]
					\centering
	...
	\level{3}{Strumenti}
		\level{4}{Latex}
		Per la stesura dei documenti si è scelto di utilizzare il \insglo{linguaggio di markup} \LaTeX{}, il quale permette di separare, in modo semplice, il contenuto di un documento dalla sua formattazione. \\
		Ogni documento \insglo{prodotto} dovrà essere realizzato a partire da un \textbf{template} \LaTeX{} appropriato reperibile al percorso \texttt{Commons/Template}.\\
		...
		\level{4}{Script per la rilevazione di errori ortografici}
		Per la rilevazione e la correzione degli errori ortografici viene utilizzato un apposito \insglo{script} realizzato dagli \insrole{Amministratori} del \groupname{}, che si appoggia al \insglo{software} \insglo{Aspell}, il quale è stato opportunamente configurato con il dizionario di lingua italiana.
		\level{4}{Script di verifica}
		I \insrole{Verificatori} hanno a disposizione gli strumenti descritti di seguito, per svolgere la parte automatizzata dell'attività di verifica.
		\begin{description}
			\item[OrtographicCheck] \insglo{Script} che sfrutta il \insglo{software} GNU \insglo{Aspell} per eseguire il controllo ortografico di un documento.
			\item[Gulpease] \insglo{Script} che estrae il testo da un documento e ne calcola l'indice di leggibilità Gulpease.
			\item[NonBreakingSpaceCheck] \insglo{Script} che controlla che un numero formattato secondo lo standard [SI/ISO 31-0] utilizzi lo spazio unificatore e non quello normale (errore in cui è facile incorrere).
		\end{description}
