	\level{2}{Processo di sviluppo}
		...
		Tali requisiti sono raccolti all'interno del documento \insdoc{Analisi dei Requisiti}. All'interno di tale documento è anche indicato un modo per verificare i requisiti in esso raccolti.
			...
			\begin{description}
				\item[\insglo{Astah}] Viene utilizzato per la creazione dei diagrammi \insglo{UML} dei casi d'uso, i quali vengono inseriti nel documento \insdoc{Analisi dei Requisiti} per descrivere i casi d'uso ricavati dall'analisi del \insglo{capitolato}.
				\begin{figure}[H]
					\centering
				...
				\end{figure}
				\item[Tracker] È un'applicazione web, appositamente creata dagli \insrole{Amministratori}, la quale deve essere utilizzata, per questa attività, per il tracciamento e la classificazione dei requisiti. Si può trovare una descrizione dettagliata nell'appendice \nameref{sec:tracker}.
			\end{description}
		...
		Approfondendo la progettazione del \insglo{prodotto} fino ad arrivare a moduli abbastanza semplici da essere capiti da una sola persona, si otterranno le istruzioni necessarie ai \insrole{Programmatori} per ottenere il \insglo{prodotto} finito.
			...
			Il modo in cui viene effettuato il tracciamento dei requisiti viene descritto in modo più approfondito nella sezione \nameref{sec:ProgTracCompReq}.
				...
				\begin{description}
					\item[\insglo{Astah}] Durante tale attività, si utilizza \insglo{Astah} per la creazione dei diagrammi \insglo{UML} di attività e dei \insglo{package}, i quali vengono inseriti nel documento \insdoc{Specifica Tecnica} per una descrizione grafica, delle attività da svolgere per realizzare certe funzionalità e delle componenti e delle loro interazioni. 
					\begin{figure}[H]
						\centering
				...
				\end{figure}
					\item[Tracker] Durante la progettazione architetturale, il Tracker, si deve utilizzare per il tracciamento delle componenti. Una descrizione più approfondita di questo strumento la si trova nell'appendice \nameref{sec:tracker}. Mentre una descrizione più approfondita delle procedure attuate tramite questa applicazione, la si può trovare nelle sezioni successive.
				\end{description}
				\level{5}{Procedure}
			...
			Il modo in cui viene effettuato il tracciamento dei requisiti viene descritto in modo più approfondito nella sezione \nameref{sec:ProgTracClasReq}.
				...
				\level{5}{Strumenti}
				Durante la progettazione di dettaglio, si deve utilizzare \insglo{Astah} per la creazione di diagrammi delle classi, di attività e di sequenza, i quali vengono aggiunti al documento \insdoc{Definizione di Prodotto}.
				\begin{figure}[H]
					\centering
	...
	\level{3}{Codifica} \label{sec:codifica}
		...
		\level{4}{Norme}
			...
			\begin{description}
					...
					\item[\insglo{Java}] I commenti devono rispettare la seguente struttura:
						...
						\end{itemize}
			\end{description}
			\level{5}{Versionamento dei file di codice} \label{sec:versionecodice}
		...
		\level{4}{Strumenti}
			...
			Per lo sviluppo dell'applicazione \insglo{Android}, invece, si utilizza l'\insglo{IDE} \ignoreglo{Android} Studio.