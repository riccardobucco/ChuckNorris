...
La progettazione ad alto livello dell'architettura e dei singoli componenti del \insglo{framework} \projectname{} viene descritta dai \insrole{Progettisti} nel documento \insdoc{Specifica Tecnica v1.00}, avendo cura di rispettare i seguenti aspetti.
\level{5}{Norme}
...
Per formalizzare ed esplicitare alcuni aspetti specifici del sistema è necessario utilizzare i seguenti diagrammi \insglo{UML}:
\begin{itemize}
\item diagrammi dei \insglo{package}, per rappresentare l'aspetto generale del sistema;
...
Per garantire la qualità del \insglo{prodotto} è fondamentale che ogni requisito sia soddisfatto da uno e un solo componente. Tale tracciamento viene generato automaticamente dall'applicazione per la gestione dei requisiti sotto forma di tabelle.
...
L'attività di codifica ha lo scopo di produrre il codice necessario alla realizzazione del \insglo{prodotto} richiesto. Il codice generato dovrà rispettare il più fedelmente possibile ciò che è stato definito nel documento \insdoc{Definizione di Prodotto v1.00}.
	...
	\begin{itemize}
		...
		\begin{description}
			...
			\item[\insglo{Package}] indica il \insglo{package} al quale appartiene il file, comprensivo di gerarchia;
