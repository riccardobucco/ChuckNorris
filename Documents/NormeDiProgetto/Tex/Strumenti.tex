\level{1}{Strumenti} \label{app:strumenti}
	...
	\level{2}{Software per i test}
		Per l'esecuzione dei test di \insglo{Norris} è stato utilizzato \textbf{Mocha}, un \insglo{framework} \insglo{JavaScript} che può essere utilizzato come modulo di \insglo{node.js}. L'esecuzione dei test è accompagnata da accurati report. Mocha permette l'utilizzo di una qualsiasi libreria di asserzioni\footnote{Una libreria che permette di fare delle vere e proprie asserzioni durante la programmazione, utilile durante la creazione dei test}: nel nostro caso si è scelto di utilizzare il modulo \texttt{assert} presente in \insglo{node.js}.\\
		Per quanto riguarda l'eseccuzione dei test di \insglo{Chuck} si è optato per \textbf{Jasmine}. Anch'esso è un \insglo{framework} \insglo{JavaScript} utile all'attività di testing. La scelta è ricaduta su di esso in quanto fortemente consigliato dai creatori di \insglo{Angular.js}.\\
		Infine, per testare l'Applicazione \insglo{Android} si è scelto l'ambiente \textbf{JUnit}. Esso è un \insglo{framework} ben integrato con l'SDK \insglo{Android}, e fornito direttamente dagli sviluppatori stessi. Esso permette la scrittura semiautomatica di buona parte del codice di cui è composto un test.