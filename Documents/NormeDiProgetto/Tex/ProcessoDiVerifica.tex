...
\level{4}{Tecniche di analisi}
		...
		I test si distinguono in varie tipologie, le quali vengono di seguito descritte. Nella sezione \nameref{sec:procedure} vengono spiegate le procedure da effettuare per eseguire i test sui prodotti realizzati.
			\level{4}{Norme}
				...
				I test di unità sono identificati da un codice univoco con la seguente sintassi: TU[Parte del prodotto]-[Nome della classe testata][Numero progressivo a due cifre]. Si noti che nel nome della classe testata non deve essere compreso il package di appartenenza, e la parte del prodotto viene indicata con:
				\begin{itemize}
					\item A, per indicare l'applicazione Android;
					\item C, per indicare la libreria Chuck;
					\item N, per indicare il framework Norris.
				\end{itemize}
			...
			Di seguito vengono descritte le procedure da seguire per effettuare due diverse tipologie di test (test di unità e test di integrazione) sui prodotti \textit{Norris} e \textit{Chuck}.
				\level{5}{Norris}
					...
					\begin{itemize}
						...
						\item In caso non sia presente, si installi il test framework “mocha”, tramite il comando \texttt{npm install -g mocha}. Per ulteriori informazioni e approfondimenti si visiti il sito \insuri{http://mochajs.org/}.
					...
					Per eseguire i test di integrazione su Norris, si procede nel modo di seguito esposto, dando per scontato che sia già installato il \textit{package manager} \texttt{npm} di node.js.
					\begin{itemize}
						\item Da terminale ci si sposti nella cartella \inspath{Sources/Norris}.
						\item In caso non sia presente, si installi il test framework “mocha”, tramite il comando \texttt{npm install -g mocha}. Per ulteriori informazioni e approfondimenti si visiti il sito \insuri{http://mochajs.org/}.
					...
					\end{itemize}
				\level{5}{Chuck}
				Per eseguire i test di unità e di integrazione su Chuck, si segua la procedura descritta di seguito.
					\begin{itemize}
					\item Da terminale ci si sposti nella cartella \inspath{Sources/Chuck}.
					\item Si digiti il comando \texttt{npm test}. La prima volta, verrà eseguito, in modo automatico, il comando \texttt{npm install} il quale installa le dipendenze in una cartella locale “node\_modules”, verrà installato, anche, il framework “jasmine” e saranno effettuati tutti i test in modo automatico. (Per maggiori informazioni e approfondimenti su “jasmine” si visiti il sito \insuri{http://jasmine.github.io/}.) \\
					Durante l'esecuzione del comando si aprirà una finestra del browser (la si può ignorare) e alla fine della procedura compariranno i risultati dei test. \\
