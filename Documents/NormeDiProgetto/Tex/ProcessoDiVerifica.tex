...
\level{4}{Procedure}
	...
	\level{4}{Strumenti}
	Per l'attività di verifica dei documenti devono essere utilizzati i seguenti appositi \insglo{script} sviluppati dagli amministratori:
	\begin{description}
...
\level{4}{Tecniche di analisi}
		...
		I test si distinguono in varie tipologie, le quali vengono di seguito descritte. Nella sezione \nameref{sec:procedure} vengono spiegate le procedure da effettuare per eseguire i test sui prodotti realizzati.
			\level{4}{Norme}
				...
				\level{5}{Test di robustezza}
				I test di robustezza servono a prevenire il collasso del sistema in situazioni limite. In particolare, poiché nel progetto viene usato principalmente un linguaggio non tipato quale il \insglo{JavaScript}, sono stati previsti dei casi in cui vengono effettuati dei controlli di tipo sui parametri passati a metodi e costruttori. Qualora tali controlli non venissero superati, il programma deve sollevare delle eccezioni indicando con messaggi significativi il tipo di errore commesso.\\
			...
			Di seguito vengono descritte le procedure da seguire per effettuare due diverse tipologie di test (test di unità e test di integrazione) sui prodotti \textit{\insglo{Norris}} e \textit{\insglo{Chuck}}.
				...
				\level{5}{Norris}
					...
					\begin{itemize}
						...
						\item In caso non sia presente, si installi il test \insglo{framework} “\insglo{mocha}”, tramite il comando \texttt{npm install -g mocha}. 
						\item Una volta installato “\insglo{mocha}”, ci si sposti nella cartella \inspath{Sources/Norris/test/unit} e si digiti, quindi, il comando \texttt{mocha nomeTest.js} dove \textit{nomeTest} indica il nome del file del test da eseguire. 
					...
					\begin{itemize}
						\item Da terminale ci si sposti nella cartella \inspath{Sources/Norris}.
						\item In caso non sia presente, si installi il test \insglo{framework} “\insglo{mocha}”, tramite il comando \texttt{npm install -g mocha}. 
						\item Una volta installato “\insglo{mocha}”, ci si sposti nella cartella \inspath{Sources/Norris/test/integration} e si digiti, quindi, il comando \texttt{mocha nomeTest.js} dove \textit{nomeTest} indica il nome del file del test da eseguire.
					...
					\level{6}{Test di robustezza}
					Per eseguire i test di robustezza su \insglo{Norris}, si procede nel modo di seguito esposto, dando per scontato che sia già installato il \textit{\insglo{package} manager} \texttt{npm} di \insglo{node.js}.
					\begin{itemize}
						\item Da terminale ci si sposti nella cartella \inspath{Sources/Norris}.
						\item In caso non sia presente, si installi il test \insglo{framework} “\insglo{mocha}”, tramite il comando \texttt{npm install -g mocha}. 
						\item Una volta installato “\insglo{mocha}”, ci si sposti nella cartella \inspath{Sources/Norris/test/strength} e si digiti, quindi, il comando \texttt{mocha nomeTest.js} dove \textit{nomeTest} indica il nome del file del test da eseguire.
						\item In caso il test dia risultato positivo, comparirà l'elenco delle funzionalità testate con il segno di spunta, come nella figura seguente.
						\begin{figure}[H]
							\centering
				...
				Per eseguire i test di unità e di integrazione su \insglo{Chuck}, si segua la \insglo{procedura} descritta di seguito.
					\begin{itemize}
						...
						\item Si digiti il comando \texttt{npm test}. La prima volta, verrà eseguito, in modo automatico, il comando \texttt{npm install} il quale installa le dipendenze in una cartella locale “node\_modules”, verrà installato, anche, il \insglo{framework} “\insglo{jasmine}” e saranno effettuati tutti i test in modo automatico. (Per maggiori informazioni e approfondimenti su “\insglo{jasmine}” si visiti il sito \insuri{http://jasmine.github.io/}.) \\
				...
				Per eseguire i test di unità e di integrazione sull'applicazione \insglo{Android}, si esegua la \insglo{procedura} descritta di seguito.
				\begin{itemize}
			...
			\begin{description}
				\item[\insglo{jasmine}] Un \insglo{framework} \insglo{JavaScript} per l'esecuzione di test su codice che utilizza AngularJS.  Per maggiori informazioni e approfondimenti su “\insglo{jasmine}” si visiti il sito \insuri{http://jasmine.github.io/}.
				\item[\insglo{mocha}] Un \insglo{framework} \insglo{Javascript} per l'esecuzione di test su applicazioni basate su \insglo{Node.js}. Per ulteriori informazioni e approfondimenti si visiti il sito \insuri{http://mochajs.org/}.
				\item[JUnit] Un \insglo{framework} per l'esecuzione di test su applicazioni \insglo{Java}. Per ulteriori informazioni e approfondimenti si visiti il sito \insuri{http://junit.org/}.
			\end{description}
		...
		\level{4}{Strumenti}
			...
			\begin{description}
				\item[\insglo{GitHub}] è stato scelto il servizio di \insglo{issue} tracking offerto da \insglo{GitHub}, come suggerito nel \insglo{capitolato}, in modo da unificare gestione delle \insglo{issue} e versionamento.
