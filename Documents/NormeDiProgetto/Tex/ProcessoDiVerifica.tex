			...
			Di seguito vengono descritte le procedure da seguire per effettuare due diverse tipologie di test (test di unità e test di integrazione) sui prodotti \textit{\insglo{Norris}} e \textit{\insglo{Chuck}}.
				\level{5}{Norris}
					...
					\begin{itemize}
						...
						\item In caso non sia presente, si installi il test \insglo{framework} “mocha”, tramite il comando \texttt{npm install -g mocha}. Per ulteriori informazioni e approfondimenti si visiti il sito \insuri{http://mochajs.org/}.
				...
				\end{itemize}
			\level{4}{Strumenti}
			Si rimanda all'appendice \nameref{app:strumenti} per una descrizione più completa degli strumenti utilizzati per eseguire i test sopra descritti.
