		...
		\level{3}{Issue tracking}
			...
			\level{4}{Procedure}
				...
				\begin{enumerate}
				    ...
				    \item il \insrole{Project Manager} valuterà la richiesta e se decide che la modifica è inutile o dannosa procederà cambiando etichetta da \inslabel{Request} a \inslabel{Rejected} e chiudendo la \insglo{issue} terminando così la \insglo{procedura};
				    \item il \insrole{Verificatore} aggiorna la lista degli errori comuni (\insglo{procedura} descritta nella sezione \nameref{sec:tracciamento}).
				    \item un qualsiasi componente del gruppo può proporre una soluzione ad una certa \insglo{issue} commentando all'interno di essa cambiando etichetta della \insglo{issue} da \inslabel{Request} a \inslabel{Questioned} permettendo così agli altri componenti di ribattere l'idea o di proporne una nuova;
				    \item quando si è deciso che la soluzione proposta sia quella ottimale l'etichetta verrà cambiata da \inslabel{Questioned} a \inslabel{ToDo} ed il \insrole{Project Manager} assegnerà il \insglo{ticket} di modifica ad un membro da lui scelto;
