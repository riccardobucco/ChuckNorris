...
\level{3}{Analisi}
		...
		\level{3}{Issue tracking}
			...
			\level{4}{Norme}
				...
				Una label è una etichetta che viene applicata ad una issue utilizzando il servizio fornito da GitHub (consultare la sezione \nameref{app:strumenti} del presente documento) permettendo così di categorizzare e filtrare le varie issues.\\
				...
				\begin{itemize}
				    ...
				    \item \inslabel{Rejected} implica una issue respinta perché la richiesta è stata giudicata inutile o dannosa;
			...
			\level{4}{Procedure}
				...
				\begin{enumerate}
				    \item il \insrole{Verificatore} crea una nuova issue su Github riguardante l'anomalia rinvenuta (seguendo la sintassi indicata nella sezione \nameref{sec:sintassiissue}) ed aggiunge come etichette \inslabel{Verifica}, quella relativa al documento/i interessato/i ed infine etichetta di stato \inslabel{Request}.
				    \item il \insrole{Project Manager} valuterà la richiesta e se decide che la modifica è inutile o dannosa procederà cambiando etichetta da \inslabel{Request} a \inslabel{Rejected} e chiudendo la issue terminando così la procedura;
				    \item il \insrole{Verificatore} aggiorna la lista degli errori comuni (procedura descritta nella sezione \nameref{sec:tracciamento}).
				    \item un qualsiasi componente del gruppo può proporre una soluzione ad una certa issue commentando all'interno di essa cambiando etichetta della issue da \inslabel{Request} a \inslabel{Questioned} permettendo così agli altri componenti di ribattere l'idea o di proporne una nuova;
				    ...
				    \item il componente scelto dal gruppo avrà il compito di modificare l'etichetta cambiandola da \inslabel{ToDo} ad \inslabel{InProgress} prendendo quindi in carico il ticket di lavoro;
