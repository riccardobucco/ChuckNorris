...
\end{itemize}
\level{3}{Svantaggi}
L'utilizzo di Socket.io porta, anche, ad alcuni svantaggi:
\begin{itemize}
	\item richiede che, sia client che server, utilizzino le librerie di Socket.io;
...
\level{3}{Svantaggi}
Non sono stati rilevati grossi svantaggi nell'uso di Express.js, il quale risulta essere un buon framework che fornisce molti metodi per estendere il modulo http di Node.js.
...
\begin{itemize}
\item è supportato solo dalle versioni più recenti dei browser;
...
\end{itemize}
\level{3}{Svantaggi}
L'unico svantaggio rilevato dall'uso di CSS3 è dato dal fatto che, essendo un linguaggio ancora in via di sviluppo, è supportato (non appieno) solo dai web browser più recenti.
...
\end{itemize}
\level{3}{Svantaggi}
Uno degli svantaggi rilevato nell'uso di Angular.js è la documentazione povera di esempi d'utilizzo. Tuttavia, questo può essere risolto tramite un'approfondita ricerca nel web. Un altro svantaggio deriva dal fatto che il codice non è valido secondo lo standard del W3C. 
...
\level{2}{DataTables}
\level{3}{Descrizione}
È stato scelto di usare la libreria JavaScript DataTables per la creazione di tabelle in HTML5. Tale libreria permette la creazione di tabelle dinamiche ed interattive. DataTables fornisce delle API per la manipolazione delle tabelle e per l'accesso ai dati contenuti in esse. 
\level{3}{Vantaggi}
L'utilizzo di DataTables offre i seguenti vantaggi:
\begin{itemize}
...
\end{itemize}
\level{3}{Svantaggi}
Nell'uso di DataTables non sono stati rilevati grossi svantaggi. DataTables offre un'ampia documentazione piena di esempi d'uso. Inoltre, grazie al forum, presente sul sito web ufficiale, si possono ottenere risposte ad eventuali problemi o dubbi che sorgono durante l'utilizzo della libreria.

\level{2}{OpenStreetMap}
\level{3}{Descrizione}
È stato scelto di utilizzare OpenStreetMap, per la visualizzazione di Map Charts, un progetto collaborativo finalizzato alla creazione di dati cartografici, liberi e gratuiti a chiunque. Il progetto è stato lanciato poichè molte delle mappe che normalmente si pensano libere, hanno in realtà restrizioni legali o tecniche. OpenStreetMap mette, inoltre, a disposizione i geodati su cui le mappe si basano, al contrario di moltre altre librerie, come Google Maps \insglo{API}.
\level{3}{Vantaggi}
...
\level{3}{Vantaggi}
L'utilizzo di GoogleMaps Android API v2 fornisce i seguenti vantaggi:
\begin{itemize}
