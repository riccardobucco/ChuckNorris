\level{2}{Chuck}
	...
	\level{3}{Descrizione delle componenti di Chuck}
		...
		La componente \insglo{Chuck} \insglo{Api} Manager si occupa di implementare le funzionalità offerte dalle \insglo{API} di \insglo{Chuck} allo sviluppatore \insglo{client}. Queste funzionalità sono:
		\begin{itemize}
			...
			\item logout.
		\end{itemize}
		Questa componente si occupa inoltre di gestire la comunicazione con \insglo{Norris}.
		
		\level{4}{Chart View}
		La componente Chart View ha il compito di visualizzare i grafici all'interno della pagina web. I grafici possono essere del tipo \insglo{Bar Chart}, \insglo{Line Chart}, \insglo{Map Chart} e \insglo{Table}. Quando un grafico viene aggiornato, questa componente si occupa di aggiornare anche la sua visualizzazione nella pagina web.
		...
		\level{4}{Data Model}
		La componente Data Model è un modello che astrae i grafici visualizzati nella pagina web. In essa sono contenuti i dati riguardanti i grafici, assieme alle relative impostazioni. In particolare sono presenti i modelli di tutte le tipologie di chart implementati da \insglo{Norris}. Il Data Model fornisce per ciascuna tipologia di grafico i metodi per inserire i dati e configurare alcune impostazioni. 
    
	\level{3}{Descrizione delle interazioni tra le componenti}
	
		\level{4}{Chuck API Manager - Data Model}
		Quando il Web Developer utilizza le \insglo{API} di \insglo{Chuck} oppure quando arriva un messaggio da \insglo{Norris}, \insglo{Chuck} \insglo{API} Manager apporta le opportune modifiche al Data Model, in modo che quest'ultimo rispecchi costantemente lo stato dei grafici da visualizzare.
	...
	Nella progettazione delle componenti di \insglo{Chuck} abbiamo deciso di utilizzare i seguenti \insglo{pattern} architetturali:
		\level{4}{Model View Controller}
		Model-View-Controller (\insglo{MVC}) è un \insglo{pattern} architetturale utilizzato per separare il codice in blocchi di funzionalità ben distinte.\\
		Per la descrizione del \insglo{pattern} e dei vantaggi derivanti dalla sua applicazione si rimanda all'appendice \nameref{app:MVC}.
			...
			L'\insglo{MVC} viene utilizzato per dividere le classi della libreria \insglo{Chuck} in tre grandi componenti:
			\begin{itemize}
				\item ChartView;
				\item ChartModel;
